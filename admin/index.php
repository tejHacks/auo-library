...
function validateMatric($adminId) {
    return preg_match("/^AU[0-9]{2}[A-Z]{2}[0-9]{4}/",$adminId);
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!validateMatric($adminId)) {
        echo "<div class='alert alert-danger mt-3' role='alert'>Invalid librarian format (use the format AU10AN1234).
    ...
    $stmt = $conn->prepare("SELECT `Password` FROM `Librarian` WHERE `LibrarianID` = ?");
    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-6">
                ...
                <form method="POST" enctype="application/x-www-form-urlencoded" class="text-center my-3 p-4 rounded border border-2 border-success" accept-charset="UTF-8" autocomplete="off"
                    id="registrationForm">

                    <div class="form-group text-left">
                        <label for="adminId">Librarian ID</label>
                        <input type="text" class="form-control" name="adminID" id="adminId" placeholder="AU24CC5099">
